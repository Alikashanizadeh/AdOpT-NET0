...
new_technologies_path = Path("./New technologies")
new_networks_path = Path("./New network")
...
topology["end_date"] = "2022-01-08 00:00"
...
# ============================================================
# 6. COPY BUILT-IN TECHNOLOGY DATA
# ============================================================
adopt.copy_technology_data(input_data_path)


# ============================================================
# 7. COPY CUSTOM TECHNOLOGY JSON FILES
# ============================================================
custom_tech_map = {
    "Hydrogenation_LOHC.json": input_data_path / "period1" / "node_data" / "hydrogenation_node" / "technology_data" / "Hydrogenation_LOHC.json",
    "Dehydrogenation_LOHC.json": input_data_path / "period1" / "node_data" / "dehydrogenation_node" / "technology_data" / "Dehydrogenation_LOHC.json",
}

for filename, target_path in custom_tech_map.items():
    source_path = new_technologies_path / filename
    if source_path.exists():
        shutil.copy(source_path, target_path)
        print(f"Copied custom technology: {source_path} -> {target_path}")
    else:
        print(f"WARNING: custom technology file not found: {source_path}")


# ============================================================
# 8. DEFINE NETWORKS
#    Replace HYDROGEN_NETWORK_NAME with the exact built-in
#    hydrogen network component name if needed.
# ============================================================
HYDROGEN_NETWORK_NAME = "hydrogenTruck"

with open(input_data_path / "period1" / "Networks.json", "r") as f:
    networks = json.load(f)

networks["new"] = [
    HYDROGEN_NETWORK_NAME,
    "lohcTransport",
...
# ============================================================
# 9. CREATE NETWORK TOPOLOGY FOLDERS
# ============================================================
...
write_network_topology(
    ...
    distances={
        ("import_hydrogen_node", "hydrogenation_node"): 50,
        ("dehydrogenation_node", "demand_node"): 50
    },
    size_max_value=1000
)

# loaded LOHC: hydrogenation -> dehydrogenation
write_network_topology(
    "lohcTransport",
    arcs=[("hydrogenation_node", "dehydrogenation_node")],
    distances={("hydrogenation_node", "dehydrogenation_node"): 50},
    size_max_value=1000
)

# unloaded LOHC: dehydrogenation -> hydrogenation
write_network_topology(
    "unloaded_lohcTransport",
    arcs=[("dehydrogenation_node", "hydrogenation_node")],
    distances={("dehydrogenation_node", "hydrogenation_node"): 50},
    size_max_value=1000
)
...
# ============================================================
# 10. COPY NETWORK DATA
# ============================================================
adopt.copy_network_data(input_data_path)

custom_network_map = {
    "lohcTransport.json": input_data_path / "period1" / "network_data" / "lohcTransport.json",
    "unloaded_lohcTransport.json": input_data_path / "period1" / "network_data" / "unloaded_lohcTransport.json",
}

for filename, target_path in custom_network_map.items():
    source_path = new_networks_path / filename
    if source_path.exists():
        shutil.copy(source_path, target_path)
        print(f"Copied custom network: {source_path} -> {target_path}")
    else:
        print(f"WARNING: custom network file not found: {source_path}")


# ============================================================
# 11. SIMPLE CARRIER DATA
# ============================================================
hours = 24 * 7
...
adopt.fill_carrier_data(
    input_data_path,
    value_or_data=[100] * hours,
    columns=["Demand"],
    carriers=["hydrogen"],
    nodes=["demand_node"]
)

# temporary unloaded LOHC source at hydrogenation node
adopt.fill_carrier_data(
    input_data_path,
    value_or_data=1000,
    columns=["Import limit"],
