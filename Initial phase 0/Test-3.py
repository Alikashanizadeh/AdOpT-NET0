...
custom_tech_path = Path(r"/adopt_net0/database/templates/technology_data/New Technologies")
custom_network_path = Path(r"C:\Users\kasha\Thesis\Main optimisation file\AdOpT-NET0\New Networks")
# -----------------------------
...
# Turn off clustering for first debugging
...
zero_series = pd.Series([0.0] * hours)

hydrogen_import_limit = pd.Series([1000.0] * hours)
hydrogen_import_price = pd.Series([50.0] * hours)

heat_import_limit = pd.Series([1000.0] * hours)
heat_import_price = pd.Series([10.0] * hours)

lohc_unloaded_import_limit = pd.Series([1000.0] * hours)
lohc_unloaded_import_price = pd.Series([1.0] * hours)

lohc_unloaded_export_limit = pd.Series([1000.0] * hours)
lohc_unloaded_export_price = pd.Series([0.0] * hours)
...
adopt.fill_carrier_data(
    input_data_path,
    value_or_data=hydrogen_import_limit,
...
# No lohc_loaded imports anywhere
for node in ["source", "demand"]:
    adopt.fill_carrier_data(
        input_data_path,
        value_or_data=zero_series,
...
# Allow lohc_unloaded import at source only
adopt.fill_carrier_data(
    input_data_path,
    value_or_data=lohc_unloaded_import_limit,
    columns=["Import limit"],
...
adopt.fill_carrier_data(
    input_data_path,
    value_or_data=hydrogen_import_price,
...
adopt.fill_carrier_data(
    input_data_path,
    value_or_data=heat_import_price,
    columns=["Import price"],
    carriers=["heat"],
    nodes=["demand"]
)
...
# lohc_unloaded import price at source only
adopt.fill_carrier_data(
    input_data_path,
    value_or_data=lohc_unloaded_import_price,
    columns=["Import price"],
    carriers=["lohc_unloaded"],
    nodes=["source"]
)
...
# ----- Export limits -----
for node in ["source", "demand"]:
    for carrier in ["hydrogen", "heat", "lohc_loaded"]:
        adopt.fill_carrier_data(
            input_data_path,
            value_or_data=zero_series,
...
# No lohc_unloaded export at source
adopt.fill_carrier_data(
    input_data_path,
    value_or_data=zero_series,
    columns=["Export limit"],
...
# Allow lohc_unloaded export at demand
adopt.fill_carrier_data(
    input_data_path,
    value_or_data=lohc_unloaded_export_limit,
    columns=["Export limit"],
    carriers=["lohc_unloaded"],
    nodes=["demand"]
)
...
# lohc_unloaded export at demand
adopt.fill_carrier_data(
    input_data_path,
    value_or_data=lohc_unloaded_export_price,
    columns=["Export price"],
    carriers=["lohc_unloaded"],
    nodes=["demand"]
)
